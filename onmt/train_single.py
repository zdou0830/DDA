...
def main(opt):
    ...
    if opt.lm_deep_fusion:
        logger.info('Loading language model checkpoint from %s' % opt.lm_in)
        fuse_lm_checkpoint = torch.load(opt.lm_in,
                                map_location=lambda storage, loc: storage)
    else:
        fuse_lm_checkpoint = None

    if opt.lm_my_deep_fusion:
        logger.info('Loading language model checkpoint from %s' % opt.lm_fusion)
        fuse_lm_checkpoint = torch.load(opt.lm_in,
                                map_location=lambda storage, loc: storage)
        fuse_lm_out_checkpoint = torch.load(opt.lm_out,
                                map_location=lambda storage, loc: storage)
    else:
        fuse_lm_checkpoint = None
        fuse_lm_out_checkpoint = None

    if opt.lm_multi:
        logger.info('Loading language model checkpoint from %s' % opt.lm_multi)
        lm_multi_checkpoint = torch.load(opt.lm_multi,
                                map_location=lambda storage, loc: storage)
    else:
        lm_multi_checkpoint = None
    ...
    # Build model.
    model = build_model(model_opt, opt, fields, checkpoint, fuse_lm_checkpoint=fuse_lm_checkpoint, lm_multi_checkpoint=lm_multi_checkpoint, fuse_lm_out_checkpoint=fuse_lm_out_checkpoint)
