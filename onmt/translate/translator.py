...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    if opt.lm_bias:
        translator = LMBiasTranslator(model, fields, global_scorer=scorer,
                                ...
                                copy_attn=model_opt.copy_attn, logger=logger, bias_param=opt.bias_param,
                                **kwargs)
    elif opt.lm_deep_fusion:
        translator = LMDFTranslator(model, fields, global_scorer=scorer,
                                out_file=out_file, report_score=report_score,
...
class Translator(object):
    ...
    def _translate_batch(self, batch, data):
        ...
        for i in range(self.max_length):
            ...
            if True:
                self.weight1 += (torch.sum(abs(weight1*dec_out)))
                self.weight2 += (torch.sum(abs(weight2*lm_dec_out)))
                self.cnt += 1
                if self.cnt % 10 == 0:
                    print(self.cnt)
                    print(self.weight1)
                ...
                #self.weight1 += (torch.sum(abs(weight1)))
...
class LMBiasTranslator(object):
    ...
    def _translate_batch(self, batch, data):
        ...
        for i in range(self.max_length):
            ...
            vocab = self.fields["tgt"].vocab
            eos=vocab.stoi[inputters.EOS_WORD]
            out += self.bias_param*(lm_in_out-lm_out_out)
            out = torch.exp(out)
